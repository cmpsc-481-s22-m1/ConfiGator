...
def main(
    ...
    brk: bool = typer.Argument(False),
    ff: bool = typer.Argument(False),
...
#     typer.echo(f"Program breaks when check fails: {brk}")
#     typer.echo(f"Assignment fails at first check fail: {ff}")
#     typer.echo(f"Indentation level: {ind}")
#     typer.echo(f"GatorGradle version: {vers}")
#     typer.echo(f"Minimum required commits: {com}")
